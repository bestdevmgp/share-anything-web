...
const RecentLoginBubble: React.FC<{ label: string }> = ({ label }) => (
    <span className="absolute left-full top-1/2 -translate-y-1/2 ml-2.5 animate-bubble-pop pointer-events-none">
        <span className="absolute top-1/2 -left-[7px] -translate-y-1/2 w-3.5 h-3.5 bg-popover border-l border-b border-border rotate-45" />
        <span className="relative block bg-popover border border-border rounded-xl shadow-lg px-3 py-1.5 text-xs font-medium text-popover-foreground whitespace-nowrap">
            {label}
        </span>
    </span>
);
