...
type OAuthProvider = 'google' | 'naver' | 'kakao' | 'apple';
...
const LoginPage: React.FC = () => {
    ...
    useEffect(() => {
        const saved = localStorage.getItem('lastLoginProvider') as OAuthProvider | null;
        if (saved && ['google', 'naver', 'kakao', 'apple'].includes(saved)) {
...
const RecentLoginBubble: React.FC<{ label: string }> = ({ label }) => (
    <div className="absolute right-0 top-1/2 translate-x-[calc(100%+10px)] -translate-y-1/2 animate-bubble-pop pointer-events-none max-sm:translate-x-0 max-sm:right-3 max-sm:-top-2 max-sm:-translate-y-full">
        {/* Arrow pointing left (hidden on small screens) */}
        <div className="absolute top-1/2 -left-[6px] -translate-y-1/2 w-3 h-3 bg-popover border-l border-b border-border rotate-45 max-sm:hidden" />
        {/* Arrow pointing down (shown on small screens) */}
        <div className="hidden max-sm:block absolute -bottom-[6px] right-4 w-3 h-3 bg-popover border-r border-b border-border rotate-45" />
        <div className="bg-popover border border-border rounded-lg shadow-lg px-3 py-1.5 text-xs font-medium text-popover-foreground whitespace-nowrap">
            {label}
        </div>
    </div>
);
