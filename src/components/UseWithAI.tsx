...
const MarkdownIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 text-foreground/70" fill="currentColor" aria-hidden="true">
...
const ClaudeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0" fill="#D97757" aria-hidden="true">
...
// OpenAI / ChatGPT mark.
const ChatGptIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 text-foreground/80" fill="currentColor" aria-hidden="true">
...
const UseWithAI: React.FC<Props> = ({ markdown, url, promptIntro, t }) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className="group inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-2 text-sm font-medium text-foreground can-hover:hover:bg-accent active:bg-accent data-[state=open]:bg-accent transition-colors focus:outline-none">
      ...
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2 text-sm" onClick={copyMarkdown}>
          <MarkdownIcon />
          <span>{t('cli.copyMarkdown')}</span>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2 text-sm" onClick={() => openInAI('https://claude.ai/new?q=')}>
          <ClaudeIcon />
          <span className="flex items-center gap-1">
            {t('cli.openInClaude')}
            <ArrowUpRightIcon className="w-3 h-3 text-muted-foreground" strokeWidth={2.5} />
          </span>
        </DropdownMenuItem>
        <DropdownMenuItem className="cursor-pointer gap-2 px-3 py-2 text-sm" onClick={() => openInAI('https://chatgpt.com/?q=')}>
          <ChatGptIcon />
          <span className="flex items-center gap-1">
            {t('cli.openInChatGPT')}
            <ArrowUpRightIcon className="w-3 h-3 text-muted-foreground" strokeWidth={2.5} />
