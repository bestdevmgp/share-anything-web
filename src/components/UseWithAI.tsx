...
const MarkdownIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 text-foreground/70" fill="currentColor" aria-hidden="true">
...
const ClaudeIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4" fill="#D97757" aria-hidden="true">
...
// OpenAI / ChatGPT mark.
const ChatGptIcon = () => (
  <svg viewBox="0 0 24 24" className="w-4 h-4 text-foreground/80" fill="currentColor" aria-hidden="true">
...
const IconBox: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <span className="flex items-center justify-center w-9 h-9 shrink-0 rounded border border-border">
    {children}
  </span>
);
...
const UseWithAI: React.FC<Props> = ({ markdown, url, promptIntro, t }) => {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" sideOffset={6} className="flex flex-col w-auto p-0 rounded-none border-border bg-card">
