...
const ApiTermsZhTW: React.FC = () => (
    <div className="space-y-8 text-gray-700 dark:text-[#888888]">
        ...
        <section>
            ...
            <ul className="list-disc list-inside space-y-2 ml-4">
                <li>一般上傳沒有單一檔案大小限制，單一檔案僅受下方每個 API 金鑰的儲存配額約束。大型檔案請使用分片上傳。P2P 安全傳輸不受任何大小限制。</li>
                <li>每個 API 金鑰下，所有尚未過期的分享（分享代碼仍有效且未被刪除的全部分享）的檔案大小總和上限為 <strong>500GB</strong>。超出配額時回傳 HTTP <code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">429 Too Many Requests</code> 與錯誤代碼 <code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">storage_quota_exceeded</code>。刪除既有分享或等待其過期即可收回配額。該上限僅適用於透過 OpenAPI（API 金鑰，<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sak_</code>）上傳的檔案，不適用於個人權杖（<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sat_</code>）上傳的檔案。</li>
