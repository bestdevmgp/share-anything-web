...
const ApiTermsJa: React.FC = () => (
    <div className="space-y-8 text-gray-700 dark:text-[#888888]">
        ...
        <section>
            ...
            <ul className="list-disc list-inside space-y-2 ml-4">
                <li>通常アップロードに1ファイルあたりのサイズ上限はなく、1ファイルは下記のAPIキーごとのストレージ上限の範囲内に限られます。大きなファイルにはマルチパートアップロードをご利用ください。P2Pセキュア転送にサイズ制限はありません。</li>
                <li>APIキーごとに、まだ有効期限が切れていない共有（削除されておらず、共有コードが有効なすべての共有）のファイルサイズ合計は<strong>500GB</strong>までです。超過時はHTTP <code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">429 Too Many Requests</code>と<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">storage_quota_exceeded</code>エラーコードが返されます。既存の共有を削除するか、有効期限切れを待つことで容量が再び確保されます。本上限はOpenAPI（APIキー、<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sak_</code>）でアップロードされたファイルにのみ適用され、個人トークン（<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sat_</code>）には適用されません。</li>
