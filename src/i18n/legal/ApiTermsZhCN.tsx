...
const ApiTermsZhCN: React.FC = () => (
    <div className="space-y-8 text-gray-700 dark:text-[#888888]">
        ...
        <section>
            ...
            <ul className="list-disc list-inside space-y-2 ml-4">
                <li>普通上传没有单文件大小限制，单个文件仅受下方每个 API 密钥的存储配额约束。大文件请使用分片上传。P2P 安全传输不受任何大小限制。</li>
                <li>每个 API 密钥下，所有尚未过期的分享（分享码仍有效且未被删除的全部分享）的文件大小总和上限为 <strong>500GB</strong>。超出配额时返回 HTTP <code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">429 Too Many Requests</code> 与错误代码 <code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">storage_quota_exceeded</code>。删除已有分享或等待其过期即可回收配额。该上限仅适用于通过 OpenAPI（API 密钥，<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sak_</code>）上传的文件，不适用于个人令牌（<code className="bg-gray-100 dark:bg-white/10 px-1 py-0.5 rounded text-sm font-mono">sat_</code>）上传的文件。</li>
